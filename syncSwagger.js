...
const swaggerOption = {
  url: "http://localhost:3000/swaggerApi.json",
  outputPath: "./routes",
  blacklist: blacklist,
};
const routerMap = [];
...
const main = {
  ...
  async parse() {
    ...
    this.traverse(paths);
    setTimeout(async () => {
      await writeFile('./public/routerMap.json',JSON.stringify(routerMap,null,4),err=>{
        
      }).finally(()=>{console.log('路由生成成功'); process.exit(0)})
    }, 1000);
  },
